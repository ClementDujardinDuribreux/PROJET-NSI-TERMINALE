...
class Sql:
    ...
    def get_max_score_all_player(cls):
        cls.cursor.execute("SELECT login FROM Joueurs")
        dico_scores = {}
        for login in cls.cursor.fetchall():
            cls.cursor.execute("SELECT MAX(score) FROM Scores WHERE id_player = ?", (str(cls.get_id_player(login[0]))))
            dico_scores[login[0]] = cls.cursor.fetchall()[0][0]
        return dico_scores
